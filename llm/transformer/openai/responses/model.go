...
func (tool Tool) MarshalJSON() ([]byte, error) {
	...
	if tool.Type == "function" && len(tool.Parameters) == 0 {
		// raw was successfully marshaled from the concrete toolWire struct, so it
		// is necessarily a non-empty JSON object. Avoid a second marshal of an
		// empty map on this hot request path while preserving nil as JSON null.
...
type Item struct {
	...
	Residual json.RawMessage `json:"-"`
	// Raw is an exact opaque Responses item identity. It is used only when a
	// canonical unknown item returns to the same protocol; its bytes win over
	// any decoded convenience fields so empty/null known names cannot vanish.
	Raw json.RawMessage `json:"-"`

	// The ID of the item, generated by the server.
	ID string `json:"id,omitempty"`

	// Any of "message", "agent_message", "input_text", "input_image", "input_audio", "output_text", "compaction", "compaction_summary", "context_compaction",
	...
	// Agent-message routing fields. They are meaningful only for
	// type="agent_message" and deliberately remain separate from Role: Codex
	// uses them as inter-agent boundaries, not ordinary assistant metadata.
	...
	// statusPresent distinguishes an explicit null status from absence for typed
	// unions that forbid the status member altogether. It is decode metadata
	// only and never enters the public model or wire JSON.
	statusPresent bool
...
func (item *Item) UnmarshalJSON(data []byte) error {
	...
	*item = Item(raw.itemAlias)
	item.statusPresent = len(raw.Status) > 0
	if item.statusPresent {
		item.Status = nil
		if !bytes.Equal(raw.Status, []byte("null")) {
			var status string
...
func (item Item) MarshalJSON() ([]byte, error) {
	if len(item.Raw) > 0 {
		if !json.Valid(item.Raw) {
			return nil, fmt.Errorf("opaque Responses item raw identity must be valid JSON")
		}
		// Unknown canonical items have no mutation contract. Preserving the exact
		// raw object is safer than reconstructing known optional fields through a
		// typed struct, which would erase []/null/empty values under omitempty.
