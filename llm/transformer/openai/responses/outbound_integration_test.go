...
func TestResponsesTransformer_CitationAnnotations_RoundTripIntegration(t *testing.T) {
	...
	require.Empty(t, contentItems[0].Annotations)
	require.Equal(t, "output_text", contentItems[1].Type)
	require.Equal(t, "Beta", contentItems[1].Text)
	require.Len(t, contentItems[1].Annotations, 1)
	require.Equal(t, "url_citation", contentItems[1].Annotations[0].Type)
	require.NotNil(t, contentItems[1].Annotations[0].StartIndex)
	require.NotNil(t, contentItems[1].Annotations[0].EndIndex)
	require.EqualValues(t, 0, *contentItems[1].Annotations[0].StartIndex)
	require.EqualValues(t, 4, *contentItems[1].Annotations[0].EndIndex)
	require.NotNil(t, contentItems[1].Annotations[0].URLCitation)
	require.Equal(t, "https://example.com/beta", contentItems[1].Annotations[0].URLCitation.URL)
	require.Equal(t, "Beta Source", contentItems[1].Annotations[0].URLCitation.Title)
}
